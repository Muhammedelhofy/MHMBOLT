...
const { buildFleetContext, hasOverrideAttempt, assertsFleetFigure, isPresenceQuery, looksFleet, isGreetingOpener, isWeekRangeQuery, recentlyDiscussedFleet } = require("./fleet");
...
// ── BUILD-176 (step 3): CAPABILITY-GROUNDED "never lie I can't" guarantee ──────
// The routing gate can still miss a phrasing; when it does, the turn falls to this
// general LLM — which historically hallucinated "I can't set reminders / track
// expenses" because the prompt never told it these tools EXIST. This enumerated
// ability list + hard never-decline rule is injected into EVERY prompt (spec §3.5
// layer 1), so even a total routing miss cannot produce a false capability denial.
const M8_PROMPT_ABILITIES = `WHAT YOU CAN ACTUALLY DO — real, working tools (NEVER deny these exist):
• Reminders & tasks — add, list, complete, snooze, delete (e.g. "remind me to call the bank tomorrow", "what's on my list?").
• Wallet — log, edit, and total his personal/household expenses (e.g. "add 50 sar lunch", "how much did I spend this month?").
• Fleet — driver earnings, rankings, the daily brief, tiers, cash collection, projections (from his live Bolt data).
• Business finance — P&L, margins, unit economics.
• Driver profiles — set or update a driver's rental, salary, or fuel cost.
• Notes — save and recall quick notes.
• Knowledge & web — answer from his OWN ingested docs/books/CV, and look things up online (weather, scores, prices, news).
• Travel — find flights, hotels, restaurants and plan trips, then hand him BOOKING LINKS. M8 NEVER books or pays — he confirms and pays on the airline/hotel site.
