...
if (($si -ge 0) -and ($ei -gt $si)) {
  ...
  # Build-R1 widened the keyword select to carry citation columns (id, source_doc_id,
  # source_class) for cited recall — confidence stays present for the IDF tie-break below.
  Check "select includes confidence (needed for tie-break)" ($fn.Contains('confidence, source_doc_id, source_class")') -and $fn.Contains('select("id, label, content, kind, confidence'))
