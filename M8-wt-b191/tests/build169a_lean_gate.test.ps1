...
# the veto must sit inside handleWalletCommand, AFTER the B-157 fleet/finance gate.
# Scoped to the function body (not a fixed char window) so later gates inserted between
# them (e.g. B-180's business-P&L deferral) don't stale-fail this check — order is what
# matters, not adjacency.
$fnBody = [regex]::Match($orch2, '(?s)async function handleWalletCommand\b.*?(?=\r?\nasync function |\r?\nfunction )').Value
$gateIdx = $fnBody.IndexOf('arb.domain === "fleet" || arb.domain === "finance"')
$leanIdx = $fnBody.IndexOf('lean_gated')
$vetoPlaced = ($gateIdx -ge 0) -and ($leanIdx -gt $gateIdx) -and ($fnBody.Substring($leanIdx) -match '^\S*\s*return null;')
Check "b7 veto placed after the B-157 central gate"       ($vetoPlaced)
