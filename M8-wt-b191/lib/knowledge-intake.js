...
// B-191 (FIX 2, layer a) — a knowledge source ingested from the Obsidian vault is
// titled "vault:<path>.md" (tools/vault-ingest.js). That internal namespace token must
// NEVER be composed into the model prompt (it leaked verbatim into a live reply on
// 2026-07-08). Relabel it to just the note name before it becomes a citation. Non-vault
// titles pass through untouched. Pure — the PS-5.1 mirror asserts the same transform.
function sanitizeSourceTitle(title) {
  let t = String(title || "").trim();
  const m = /^vault:(.+)$/i.exec(t);
  if (m) {
    t = m[1].split(/[\\/]/).pop();               // drop the vault: prefix + any path, keep the note name
    t = t.replace(/\.(?:md|markdown|txt)$/i, ""); // drop the note file extension
  }
  return t;
}
...
function formatCitation(metadata, title) {
  title = sanitizeSourceTitle(title);   // B-191: never let a raw "vault:<path>" title reach the prompt
  const md = metadata || {};
  const c  = md.citation || {};
...
module.exports = {
  ...
  formatCitation, sanitizeSourceTitle, renderKgHit, buildSourceCardShape,
  getSourceCard,
  // Build-158: targeted embedding backfill for knowledge-source nodes
