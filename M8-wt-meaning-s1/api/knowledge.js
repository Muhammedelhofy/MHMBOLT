...
const sourceCard        = require("../lib/handlers/source-card");   // Build-R1 (D5)
const inquiries         = require("../lib/handlers/inquiries");     // Build-R6 (inquiry ledger)
...
module.exports = async (req, res) => {
  ...
  switch (fn) {
    ...
    // Build-R1 (D5): read-only source card — citation + node counts + sample claims.
    // GET ?fn=source-card&id=<N>. No LLM call, no write. Rides the 12-fn router (cap FULL).
    case "source-card":        return sourceCard(req, res);
    // Build-R6 (inquiry ledger): read-only standing-question threads (list, or the
    // cited/assembled thread for ?q=/&thread=). No LLM call, no write. Same 12-fn router.
    case "inquiries":          return inquiries(req, res);
