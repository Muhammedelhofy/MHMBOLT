...
// B-192 (Meaning-First v3 S1): flush the understand() SHADOW after the reply is
// sent — Vercel keeps the invocation alive until this handler's promise settles,
// so the shadow's Groq call + telemetry insert survive the lambda freeze while
// adding ZERO latency and ZERO bytes to the reply.
const { flushShadow } = require("../understand");
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // B-192 S1 — runs AFTER the response above was already written; the shadow
    // can never touch the reply. No-op when M8_UNDERSTAND=off / nothing pending.
    try { await flushShadow(4000); } catch (_) {}
  }
};
