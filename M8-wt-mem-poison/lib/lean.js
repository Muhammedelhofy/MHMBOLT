...
async function runLeanTurn({ leanProbe, meta, log } = {}) {
  ...
  const draft = async (priorError) => {
    ...
    const raw = await generateOnce({
      ...
      // thinkingBudget:0 is EXPLICIT here (not just relying on llm.js's
      // omit-the-field default) — gemini-2.5-flash spends dynamic/auto
      // thinking tokens even when thinkingConfig is absent from the request
      // (~650-670 of a 700 cap, confirmed live 2026-07-06), which truncated
      // the Lean draft mid-statement at unpredictable points. Omitting the
      // field only actually means "off" for 2.0/1.5-flash; 2.5-flash needs
      // the explicit zero. maxOutputTokens raised for headroom now that the
      // budget isn't shared with invisible thinking tokens.
      genConfig: { temperature: 0, maxOutputTokens: 1024, thinkingBudget: 0 },
      meta,
    });
