...
// B-194: the recall trust floor (mirror of memory.js RECALL_MIN_TRUST). A row
// below it is a non-authoritative user assertion (a laundered question / an
// unverified system-self-claim). It is stored is_current=false so it is already
// out of the query below, but we ALSO fence it in JS so a demoted row can never
// be pulled into consolidation or become a canonical fact — even if some future
// path leaves one is_current. NULL trust is kept (legacy rows, treated as clean).
const CONSOLIDATOR_MIN_TRUST = 3;
...
async function fetchFacts(db) {
  ...
  return (res.data || []).filter(
    (r) => !(r && typeof r.trust_level === "number" && r.trust_level < CONSOLIDATOR_MIN_TRUST)
  );
}
