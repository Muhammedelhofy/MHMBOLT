...
function Resolve-Intent([string]$msg, $opts) {
  ...
  # B-191 (FIX 1) mirror: wallet<->travel PRESENT-level tie (score 1 each) -> travel wins.
  # A trip budget is planning, not an expense query. Only the 1-1 present tie flips; a
  # STRONG wallet (score 2) still wins via the strong path (guarded on top -eq 1).
  if ($pick.ambiguous -and ([int]$pick.top -eq 1) -and (
        (($pick.domain -eq 'wallet') -and ($pick.runnerUp -eq 'travel')) -or
        (($pick.domain -eq 'travel') -and ($pick.runnerUp -eq 'wallet')))) {
    return @{ domain = 'travel'; band = 'medium'; runnerUp = 'wallet'; why = 'wallet_travel_present_tie' }
  }
...
# Meaning-First v2 (S2 step 1): the abilities BODY moved from orchestrator.js to the
# CAPABILITIES single source in capability-registry.js (composed by buildAbilitiesPrompt);
# orchestrator now injects M8_PROMPT_ABILITIES = _abilities.buildAbilitiesPrompt(). Grep $cap.
Assert-True "abilities body composed from capability-registry"    ($cap -match 'buildAbilitiesPrompt')
Assert-True "orchestrator injects the composed abilities prompt"  ($orch -match 'buildAbilitiesPrompt\(\)')
Assert-True "abilities list forbids denying reminders"           ($cap -match "NEVER claim you lack")
Assert-True "abilities list names reminders/expenses/fleet"       (($cap -match 'Reminders & tasks') -and ($cap -match 'Wallet') -and ($cap -match 'Fleet'))
...
# v2 (S2 step 1): the travel ability line moved to the CAPABILITIES source; grep $cap.
Assert-True "ABILITIES: travel line + payment boundary"             (($cap -match '"Travel"') -and ($cap -match 'BOOKING LINKS') -and ($cap -match 'NEVER books or pays'))
