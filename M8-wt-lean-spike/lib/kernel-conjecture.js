  ...
  try {
    ...
    if (!dec || !dec.kernel) {
      ...
      if (drProposerExamplesEnabled()) {
        ...
        if (rawPkg.best) return renderBestKernel(rawKernel, rawPkg);
        ...
        if (singleClaim) {
          ...
          if (singleResult.holds) return renderKernelConjecture(rawKernel, singleClaim, singleResult);
        }
      }
    ...
    const pkg = await bestKernelConjecture(dec.kernel);
    return split + renderBestKernel(dec.kernel, pkg);
  } catch (e) {
