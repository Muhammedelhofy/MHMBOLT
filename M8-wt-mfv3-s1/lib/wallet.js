...
// ── B-191 (FIX 3): honest "balance" answer ──────────────────────────────────
// The wallet models SPENDING (a log of expenses), NOT a running/bank balance. A
// "what's my balance / how much is left / how much do I have" turn used to fall through
// the summary lane and hit the generic add/edit/total card — misleading, because it
// implied a balance exists. These helpers let the caller answer TRUTHFULLY per the
// CAPABILITIES honesty rules: name the limitation, then give the real spend total.
// Pure detector + pure formatters ⇒ PS-5.1-mirror testable (no DB).
//
// isBalanceQuery: a balance/"how much left/have" ask. Deliberately does NOT match
// "how much did I SPEND" (that IS a spend query the summary lane answers) — a "spend"/
// "صرف" verb vetoes the match.
const _BALANCE_RE = /\bbalance\b|\bhow much (?:is|do i have|have i got|is (?:there )?)?\s*(?:left|remaining|in (?:my|the) wallet|do i have)\b|\bhow much (?:money )?(?:do i|have i|i)\s*(?:have|got)\b|\bwhat(?:'s| is| do i have)\b[^?]*\bleft\b|رصيد|كم (?:باقي|متبقي|فلوس عندي|فلوسي)|كم عندي/i;
const _SPEND_VERB_RE = /\bspen[dt]\b|\bصرف/i;
function isBalanceQuery(message) {
  const s = String(message || "");
  if (_SPEND_VERB_RE.test(s)) return false;   // a spend query, not a balance query
  return _BALANCE_RE.test(s);
}

// Honest answer WITH the real month spend total (the caller fetched getSummary()).
function balanceReply({ expense, base, ar }) {
  const amt = round2(expense);
  const cur = String(base || "SAR");
  return ar
    ? `أنا أتابع مصروفك، مو رصيد جاري — المحفظة تسجّل المصاريف، ما تحفظ رصيد بنكي. صرفت ${amt} ${cur} هذا الشهر. اسألني «كم صرفت هذا الشهر؟» لأي تفصيل.`
    : `I track your spending, not a running balance — the wallet logs expenses, it doesn't hold a bank balance. You've spent ${amt} ${cur} this month. Ask “how much did I spend this month?” for the breakdown.`;
}

// Honest fallback when the total can't be fetched — still never implies a balance exists.
function balanceReplyNoData(ar) {
  return ar
    ? "أنا أتابع مصروفك، مو رصيد جاري — المحفظة تسجّل المصاريف وما تحفظ رصيد بنكي. اسألني «كم صرفت هذا الشهر؟» وأجمعها لك."
    : "I track your spending, not a running balance — the wallet logs expenses, it doesn't hold a bank balance. Ask “how much did I spend this month?” and I'll total it.";
}
...
module.exports = {
  ...
  isBalanceQuery, balanceReply, balanceReplyNoData,  // B-191 FIX 3
};
