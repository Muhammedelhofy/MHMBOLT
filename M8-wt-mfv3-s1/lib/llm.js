...
function buildGeminiConfig(systemInstruction, genConfig) {
  ...
  // else the GEMINI_THINKING_BUDGET env; else omitted (unset).
  const explicitThinkingBudget = genConfig?.thinkingBudget != null
    ? genConfig.thinkingBudget
    : (process.env.GEMINI_THINKING_BUDGET ? parseInt(process.env.GEMINI_THINKING_BUDGET, 10) : null);
  // Send thinkingConfig whenever a budget was EXPLICITLY requested — including
  // an explicit 0. gemini-2.5-flash defaults to DYNAMIC (non-zero, auto)
  // thinking whenever thinkingConfig is omitted entirely; that silently burned
  // ~650-670 of a 700-token cap on invisible "thoughts" and truncated the Lean
  // formalize draft mid-statement (confirmed live 2026-07-06). So "0 means off"
  // only holds if 0 is actually sent — omitting the field is NOT equivalent for
  // 2.5-flash the way the old comment assumed (it IS still equivalent for
  // 2.0/1.5-flash, which reject the field outright — hence: still omit the
  // field completely when NOTHING was specified, so those models don't 400).
  if (explicitThinkingBudget != null) config.thinkingConfig = { thinkingBudget: explicitThinkingBudget };
...
module.exports = { generate, generateStream, generateOnce, groqQuirks, extractUsage, buildGeminiConfig };
