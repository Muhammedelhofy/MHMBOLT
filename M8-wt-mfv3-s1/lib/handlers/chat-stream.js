...
// B-192 (Meaning-First v3 S1): flush the understand() SHADOW after the stream has
// ended — zero latency / zero bytes on the live reply; see chat-buffered.js.
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    try { res.write("data: [DONE]\n\n"); } catch (_) {}
    try { res.end(); } catch (_) {}
    // B-192 S1 — after res.end(): the SSE stream is closed, the shadow can't
    // touch it. No-op when M8_UNDERSTAND=off / nothing pending.
