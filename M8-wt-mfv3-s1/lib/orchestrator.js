...
const { stripSourceTokens } = require("./source-token-filter"); // B-191 FIX 2 (b): scrub leaked "vault:<path>" tokens from the final reply
...
const _understand = require("./understand"); // B-192 (Meaning-First v3 S1) — semantic pass, SHADOW only
...
function capabilityFallback(message, arb, intent) {
  ...
  if (d === "wallet") {
    ...
    // B-191 (FIX 3): a "what's my balance / how much is left" ask. The wallet models
    // SPENDING, not a running balance, so the generic card here implied a balance that
    // doesn't exist. Answer honestly. balance:true tells the caller to enrich the reply
    // with the real month spend total (async getSummary); the reply set here is the
    // honest no-number fallback if that fetch fails — it NEVER implies a balance exists.
    if (_wallet.isBalanceQuery(m)) {
      return { money: true, lane: "money", balance: true, reply: _wallet.balanceReplyNoData(ar) };
    }
    return {
      money: true,
      lane: "money",
...
// B-191 (FIX 3): enrich a balance card with the real month spend total. capabilityFallback
// is pure/sync; the getSummary() fetch lives here (both call sites are async). Best-effort —
// on any read failure the card keeps its honest no-number fallback. Mutates + returns `cap`.
async function _enrichBalanceCard(cap, message) {
  if (!cap || !cap.balance) return cap;
  try {
    const sum = await _wallet.getSummary();
    if (sum && sum.ok) cap.reply = _wallet.balanceReply({ expense: sum.expense, base: sum.base, ar: isArabic(message) });
  } catch (_) { /* keep the honest no-number fallback */ }
  return cap;
}
...
async function resolveDomainRoute(baseMessage, history) {
  try {
    ...
    // ── B-192 (Meaning-First v3 S1): the understand() SHADOW ──────────────────
    // Launched NEXT TO the live decision, never awaited here — the semantic pass
    // runs while the reply is being generated, and the HTTP handlers flush it
    // AFTER the reply bytes are sent (flushShadow). It compares understand()'s
    // meaning read against this function's actual verdict and logs both to
    // m8_router_misses (lane understand:*). It reads NOTHING back into the turn:
    // with M8_UNDERSTAND=shadow (default) or off, the reply is byte-identical.
    // resolveDomainRoute runs ONCE per message (orchestrateStream delegates with
    // precomputedRoute), so this fires exactly once per real turn.
    try { _understand.startShadow(baseMessage, history, { arb, intent, lookup }); } catch (_) {}
...
async function orchestrate({ message, sessionId, history, attachments, precomputedRoute }) {
  ...
  try {
    ...
    if (intent !== INTENT.DOC) {
      ...
      if (_cap) {
        await _enrichBalanceCard(_cap, baseMessage);   // B-191 FIX 3: real month spend on a balance ask
    ...
    // B-191 (FIX 2, layer b): code-guaranteed strip of any leaked internal knowledge-
    // source token ("vault:<path>.md", incl. 〔…〕/【…】-wrapped). Runs here — after the
    // last rewrite, before ensureHealthClose/chips and the STORE — so the token can
    // never reach the user OR be persisted. No-op (byte-identical) on a clean reply.
    response = stripSourceTokens(response);
...
async function orchestrateStream({ message, sessionId, history, attachments, onChunk, onReset }) {
  ...
  try {
    ...
    if (intent !== INTENT.DOC) {
      ...
      if (_capS) { await _enrichBalanceCard(_capS, baseMessage); const r = _capS.money ? _capS.reply + MONEY_SENTINEL : _capS.reply; emit(r); return r; }
    }
    ...
    // B-191 (FIX 2, layer b): scrub any leaked "vault:<path>" source token from the
    // assembled reply before it is persisted (saveMemory) and returned as `full`. Layer
    // (a) (sanitizeSourceTitle) already keeps the token out of the prompt, so the model
    // won't emit it — this is belt-and-suspenders on the persisted/returned copy.
    response = stripSourceTokens(response);
