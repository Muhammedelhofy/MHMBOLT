...
function resolveIntent(message, opts) {
  ...
  // B-191: WALLET⇄TRAVEL PRESENT-level tie (score 1 each) → TRAVEL wins. "budget of X …
  // recommend a trip" scores wallet:1 (WALLET_PRESENT 'budget') = travel:1 (TRAVEL_PRESENT
  // 'trip'); DOMAINS-order (wallet idx5 < travel idx9) would send it to the wallet expense
  // card and swallow the trip intent. A trip budget is PLANNING, not an expense query.
  // Guarded to top===1 so a STRONG wallet ("how much did I spend on my trip to Cairo" =
  // wallet:2/travel:2) still wins the tie via the strong path — only the 1-1 present tie flips.
  if (pick.ambiguous && pick.top === 1 &&
      ((pick.domain === "wallet" && pick.runnerUp === "travel") ||
       (pick.domain === "travel" && pick.runnerUp === "wallet"))) {
    return { domain: "travel", band: "medium", confidence: 0.7, runnerUp: "wallet", why: "wallet_travel_present_tie", scores };
  }
  if (pick.domain === "chat") {
